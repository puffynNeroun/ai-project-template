...
This attempt supersedes `.forge/artifacts/TASK-0003/plan-001.md` because the previous attempt incorrectly described Builder as transitioning TASK-0003 directly from `proposed` to `in_progress`, bypassing the required human `approve_plan` gate.

The corrected lifecycle is:

1. Planner creates this plan artifact while TASK-0003 is `proposed`.
2. Human approval preserves both plan artifacts and changes TASK-0003 from `proposed` to `approved`.
3. Builder starts only after approval, verifies the task is `approved`, then changes TASK-0003 from `approved` to `in_progress`.

## Task Interpretation

TASK-0003 validates the artifact contract introduced by TASK-0002. It should add structural validation for existing live artifacts without creating runtime orchestration or lifecycle enforcement.
...
The validator should not require artifacts based on task status, determine latest attempts, enforce approval/PASS/ACCEPT chains, validate full retry-chain semantics, or enforce append-only history through Git.
...
- Require each live artifact task directory to correspond to an existing `.forge/tasks/TASK-<number>.yaml`.
...
- Update focused validator tests.
- Update approved documentation to distinguish structural validation from deferred lifecycle enforcement.

Out of scope:

- Requiring artifact presence based on task status.
- Evaluating latest attempts, active attempts, or retry chains.
- Enforcing append-only history through Git history.
- Validating human plan approval or delivery approval evidence.
- Validating whether referenced test reports passed or review reports accepted.
- Validating Markdown headings, writing quality, or report completeness.
- Adding dependencies or modifying package metadata/lockfiles.
- Adding orchestration, automatic status transitions, databases, event streams, checkpoint runtimes, web interfaces, or remote operations.
- Refactoring Forge Validator into a plugin system or multi-module architecture.

## Corrected Lifecycle

Human approval commit:

- Preserve `.forge/artifacts/TASK-0003/plan-001.md`.
- Preserve `.forge/artifacts/TASK-0003/plan-002.md`.
- Change `.forge/tasks/TASK-0003.yaml` from `proposed` to `approved`.
- Update `docs/TASKS.md` so TASK-0003 shows `approved`.
- Do not implement Validator changes during this approval step.

Builder start:

- Confirm current branch and repository state.
- Confirm `.forge/tasks/TASK-0003.yaml` status is `approved`.
- Stop as blocked if the task is not `approved`.
- Change `.forge/tasks/TASK-0003.yaml` from `approved` to `in_progress`.
- Update `docs/TASKS.md` so TASK-0003 shows `in_progress`.
- Then implement the approved Validator and documentation changes.

Builder must not transition directly from `proposed` to `in_progress`.

## Allowed Files

Human approval may modify only:
...
Existing Planner artifacts must remain append-only:
...
1. At Builder start, perform corrected preflight and lifecycle transition.
   - Confirm TASK-0003 is `approved`.
   - Change `.forge/tasks/TASK-0003.yaml` from `approved` to `in_progress`.
   - Update `docs/TASKS.md` to show TASK-0003 as `in_progress` and point Next to Tester after Builder completion.
   - Do not edit either plan artifact.
...
3. Reuse existing helpers where possible.
   - Reuse `isPlainObject`.
   - Reuse `validateRepositoryRelativePath` for path safety.
   - Reuse `isRegularFile`.
   - Reuse `hasPathEscape` and `isAbsolutePath` indirectly through `validateRepositoryRelativePath`.
   - Continue using `parseDocument` from the existing `yaml` dependency.
...
4. Add live artifact discovery.
   ...
   - Read only direct files inside each live artifact task directory; reject nested directories and unsupported entries.
...
5. Define supported filename grammar.
   ...
   - Reject `000` because attempts must be positive.
...
6. Add front matter extraction.
   ...
   - Treat missing opening delimiter, missing closing delimiter, empty front matter, YAML parse error, or non-mapping front matter as a contract error.
...
7. Add exact metadata key validation.
   ...
   - Reject fields such as `artifact_slug`, `produced_at`, `created_at`, `updated_at`, `task_status`, `current_stage`, `model`, `model_name`, `token_usage`, `cost`, `duration`, `session_id`, `commit_sha`, or `pull_request_number` as unexpected keys.
...
9. Validate safe `input_artifacts`.
   - For each input:
     ...
     - reject duplicates.
   - Check references against the discovered artifact file set and `isRegularFile`.
...
11. Wire validation into `validateRepository`.
   ...
   - Minimal approach:
     - keep `discoverTaskFiles` responsible for task file discovery;
     - either make `validateTasks` return the discovered task file list, or add a small active-task discovery helper;
   ...
   - Keep the Validator in the existing single-file architecture.
...
12. Extend test fixture helpers.
   ...
   - Keep fixtures temporary and keep validator behavior read-only.

13. Add focused tests.
   - Valid cases:
     - current repository passes with live plan artifacts under TASK-0003;
...
14. Update documentation.
   ...
   - `.forge/artifacts/README.md`: update deferred enforcement language to distinguish implemented structural validation from still-deferred lifecycle, append-only Git-history, approval, and orchestration enforcement.
   ...
   - `docs/TASKS.md`: keep status synchronized with `.forge/tasks/TASK-0003.yaml`.
...
- AC-9: Validator and artifact docs explain structural validation versus deferred lifecycle, append-only, approval, and orchestration enforcement.
- AC-10: `docs/TASKS.md` status matches `.forge/tasks/TASK-0003.yaml`; human approval moves both to `approved`, and Builder start moves both to `in_progress`.
...
- TASK-0003 status was `approved` before Builder changed it to `in_progress`;
- no protected file changed;
- no file outside TASK-0003 `allowed_files` changed, except preserved Planner artifacts;
- no build, test, or review artifact was created during Builder implementation unless explicitly required by that later role;
...
- The workflow risk from `plan-001.md` is corrected here: Builder must not start from `proposed`; human plan approval must first set the task to `approved`.
...
- Error messages may become noisy if one malformed artifact cascades into many downstream errors. Prefer useful errors while avoiding avoidable follow-on failures.
