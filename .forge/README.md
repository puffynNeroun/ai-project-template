...
The local validator lives in `tools/forge-validator`. It verifies the current v1 project, workflow, role references, task template, active task contracts, existing live artifact structures, and latest-attempt status-aware artifact presence, and it is read-only. `.forge/project.yaml` exposes reproducible install, test, and verify commands for it.
...
Formal external schemas, runtime orchestration, retry-chain validation, outcome-chain validation, append-only Git-history enforcement, human approval evidence validation, automatic status transitions, additional workflows, and additional policies are intentionally deferred.
