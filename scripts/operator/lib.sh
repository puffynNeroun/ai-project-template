...
forge_run_verify_summary() {
  local log_file="${1:-/tmp/forge-operator-verify.log}"
  local verify_exit

  if pnpm -C tools/forge-validator verify > "$log_file" 2>&1; then
    verify_exit=0
  else
    verify_exit=$?
  fi

  awk '/^ℹ tests|^ℹ pass|^ℹ fail|Forge contract validation passed/' "$log_file"

  if [ "$verify_exit" -ne 0 ]; then
    echo
    echo "Verify failed. Last 160 lines:"
    tail -160 "$log_file"
    forge_die "Forge verify failed"
  fi
}
